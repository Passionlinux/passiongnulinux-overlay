...
pkg_postinst() {
	if use cuda; then
		usermod -a -G video ollama 2>/dev/null || ewarn "Could not add ollama to video group"
	fi
	...
	if use cuda; then
	einfo "N'oubliez pas d'ajouter l'utilisateur 'ollama' au groupe 'video' :"
	einfo "    gpasswd -a ollama video"
	fi
}
